...
- [ ] Verify server logs match expected
- [ ] Check audit log entries
- [ ] Confirm email logs (if applicable)
...
- ✅ Agent response is professional and accurate
- ✅ Correct workflow is followed
- ✅ Appropriate tools are called (visible in server logs)
- ✅ Audit log shows correct status
- ✅ Emails sent to correct recipients (if applicable)

A test case **FAILS** if:
- ❌ Wrong workflow executed
- ❌ Incorrect status logged
- ❌ Email sent to wrong person or not sent
- ❌ Agent provides incorrect information
- ❌ System crashes or throws errors

---

## 🎯 Test Coverage Matrix

| Workflow | Test Case(s) | Priority | Status |
|----------|-------------|----------|--------|
| Workflow A (Auto-Approve) | TC-002, TC-006 | High | [ ] |
| Workflow B (Manager Approval) | TC-003 | High | [ ] |
| Workflow C (Rejection) | TC-004 | High | [ ] |
| Workflow D (New User) | TC-001 | High | [ ] |
| Workflow E (De-provision) | TC-007 | Medium | [ ] |
| Duplicate Detection | TC-005 | High | [ ] |
| Edge Cases | TC-008 to TC-011 | Low-Medium | [ ] |

**Total Coverage:** 7 Core Workflows + 4 Edge Cases = **11 Test Cases**
...
**High Priority Tests:** ___/7 Passed  
**Medium Priority Tests:** ___/3 Passed  
**Low Priority Tests:** ___/1 Passed
...
2. Restart server for clean state
3. Check server logs are visible

### During Testing:
1. Take screenshots of interesting results
2. Copy-paste agent responses for documentation
